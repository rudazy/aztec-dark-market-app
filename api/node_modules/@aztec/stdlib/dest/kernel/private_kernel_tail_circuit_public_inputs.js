...
export class PartialPrivateTailPublicInputsForPublic {
    ...
    getSize() {
        return this.nonRevertibleAccumulatedData.getSize() + this.revertibleAccumulatedData.getSize() + this.publicTeardownCallRequest.getSize();
    }
    get needsSetup() {
    ...
    static fromBuffer(buffer) {
        ...
        return new PartialPrivateTailPublicInputsForPublic(reader.readObject(PrivateToPublicAccumulatedData), reader.readObject(PrivateToPublicAccumulatedData), reader.readObject(PublicCallRequest));
    }
    toBuffer() {
        return serializeToBuffer(this.nonRevertibleAccumulatedData, this.revertibleAccumulatedData, this.publicTeardownCallRequest);
    }
    static empty() {
        return new PartialPrivateTailPublicInputsForPublic(PrivateToPublicAccumulatedData.empty(), PrivateToPublicAccumulatedData.empty(), PublicCallRequest.empty());
    }
}
...
export class PrivateKernelTailCircuitPublicInputs {
    ...
    getNonRevertiblePublicCallRequests() {
        return this.forPublic ? this.forPublic.nonRevertibleAccumulatedData.publicCallRequests.filter((r)=>!r.isEmpty()) : [];
    }
    ...
    getNonEmptyPrivateLogs() {
        const privateLogs = this.forPublic ? this.forPublic.nonRevertibleAccumulatedData.privateLogs.concat(this.forPublic.revertibleAccumulatedData.privateLogs) : this.forRollup.end.privateLogs;
        return privateLogs.filter((n)=>!n.isEmpty());
    }
    getNonEmptyContractClassLogsHashes() {
        const contractClassLogsHashes = this.forPublic ? this.forPublic.nonRevertibleAccumulatedData.contractClassLogsHashes.concat(this.forPublic.revertibleAccumulatedData.contractClassLogsHashes) : this.forRollup.end.contractClassLogsHashes;
        return contractClassLogsHashes.filter((h)=>!h.isEmpty());
    }
    getEmittedContractClassLogsLength() {
        const accumulateLengths = (logHashes)=>logHashes.reduce((acc, log)=>acc + log.logHash.length, 0);
        return this.forPublic ? accumulateLengths(this.forPublic.nonRevertibleAccumulatedData.contractClassLogsHashes) + accumulateLengths(this.forPublic.revertibleAccumulatedData.contractClassLogsHashes) : accumulateLengths(this.forRollup.end.contractClassLogsHashes);
    }
    getEmittedPrivateLogsLength() {
        const accumulateLengths = (logs)=>logs.reduce((acc, log)=>acc + log.emittedLength, 0);
        return this.forPublic ? accumulateLengths(this.forPublic.nonRevertibleAccumulatedData.privateLogs) + accumulateLengths(this.forPublic.revertibleAccumulatedData.privateLogs) : accumulateLengths(this.forRollup.end.privateLogs);
    }
    static fromBuffer(buffer) {
        const reader = BufferReader.asReader(buffer);
        const isForPublic = reader.readBoolean();
        return new PrivateKernelTailCircuitPublicInputs(reader.readObject(TxConstantData), reader.readObject(Gas), reader.readObject(AztecAddress), reader.readUInt64(), isForPublic ? reader.readObject(PartialPrivateTailPublicInputsForPublic) : undefined, !isForPublic ? reader.readObject(PartialPrivateTailPublicInputsForRollup) : undefined);
    }
    toBuffer() {
   ...
   */ static emptyWithNullifier() {
        ...
        return new PrivateKernelTailCircuitPublicInputs(TxConstantData.empty(), Gas.empty(), AztecAddress.ZERO, 0n, undefined, new PartialPrivateTailPublicInputsForRollup(data));
    }
}
